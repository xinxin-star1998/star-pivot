        ...
        try {
            ...
            try {
                ...
                if (!"PONG".equals(pingResult)) {
                    ...
                    RedisMonitorVO vo = new RedisMonitorVO();
                    vo.setAvailable(false);
                    vo.setMessage("Redis 连接异常：PING 命令返回异常结果");
                    return vo;
                }
            } catch (Exception e) {
                log.error("Redis 连接测试失败：无法执行 PING 命令", e);
                RedisMonitorVO vo = new RedisMonitorVO();
                vo.setAvailable(false);
                vo.setMessage("Redis 连接失败：无法连接到 Redis 服务器。请检查：1) Redis 服务是否启动；2) 连接配置是否正确（host、port、password）；3) 网络连接是否正常");
                return vo;
            }
            ...
            try {
                ...
                lastException = e;
                log.error("执行 Redis INFO 命令时发生异常", e);
            }

            // 检查 INFO 命令是否成功获取数据
            if (infoStr == null || infoStr.trim().isEmpty()) {
                String errorMessage = "无法读取 Redis 信息：INFO 命令执行失败或返回空结果";
                if (lastException != null) {
                    errorMessage += "。错误详情: " + lastException.getMessage();
                }
                errorMessage += "。请检查：1) Redis 服务是否正常运行；2) Redis 版本是否支持 INFO 命令；3) 是否有权限执行 INFO 命令";
                
                log.error("无法读取 Redis INFO：INFO 命令返回空结果，异常: {}", lastException != null ? lastException.getMessage() : "无异常信息");
                RedisMonitorVO vo = new RedisMonitorVO();
                vo.setAvailable(false);
            ...
            Map<String, String> infoMap = parseRedisInfo(infoStr);
            
            // 验证是否成功解析到关键信息
            if (infoMap.isEmpty() || !infoMap.containsKey("redis_version")) {
                log.error("无法读取 Redis INFO：解析结果为空或缺少关键字段");
                RedisMonitorVO vo = new RedisMonitorVO();
                vo.setAvailable(false);
                vo.setMessage("无法读取 Redis 信息：INFO 数据解析失败");
                return vo;
            }

            RedisMonitorVO monitorVO = new RedisMonitorVO();
            monitorVO.setAvailable(true);

            monitorVO.setVersion(infoMap.getOrDefault("redis_version", "未知"));
            monitorVO.setMode(infoMap.getOrDefault("redis_mode", "未知"));
            monitorVO.setPort(parseInt(infoMap.get("tcp_port"), 6379));
            monitorVO.setConnected(true);

            // 内存信息
            RedisMonitorVO.MemoryInfo memoryInfo = new RedisMonitorVO.MemoryInfo();
            long usedMemory = parseLong(infoMap.get("used_memory"), 0L);
            long maxMemory = parseLong(infoMap.get("maxmemory"), 0L);
            memoryInfo.setUsedMemory(usedMemory / BYTES_TO_KB / KB_TO_MB); // 转换为 MB
            memoryInfo.setMaxMemory(maxMemory > 0 ? maxMemory / BYTES_TO_KB / KB_TO_MB : 0L);
            memoryInfo.setUsage(maxMemory > 0 ? PERCENTAGE_BASE * usedMemory / maxMemory : 0.0);
            monitorVO.setMemory(memoryInfo);
...
//                    return Long.valueOf(connection.execute("DBSIZE").toString());
                ...
                // 降级方案：尝试解析 db0 字段（格式：db0:keys=100,expires=10）
                String db0Info = infoMap.get("db0");
                if (db0Info != null && db0Info.contains("keys=")) {
                    try {
                        String keysStr = db0Info.substring(db0Info.indexOf("keys=") + 5);
                        int commaIndex = keysStr.indexOf(',');
                        if (commaIndex > 0) {
                            keysStr = keysStr.substring(0, commaIndex);
                        }
                        keyInfo.setTotalKeys(parseLong(keysStr, 0L));
                    } catch (Exception e2) {
                        log.warn("解析 db0 字段失败: {}", e2.getMessage());
                        keyInfo.setTotalKeys(0L);
                    }
                } else {
                    keyInfo.setTotalKeys(0L);
                }
            }
            monitorVO.setKeys(keyInfo);

            // 命令统计
            RedisMonitorVO.CommandInfo commandInfo = new RedisMonitorVO.CommandInfo();
            long totalCommands = parseLong(infoMap.get("total_commands_processed"), 0L);
            commandInfo.setTotalCommands(totalCommands);
            // 使用 instantaneous_ops_per_sec 获取每秒命令数
            String instantOpsPerSec = infoMap.get("instantaneous_ops_per_sec");
            if (instantOpsPerSec != null) {
                commandInfo.setCommandsPerSecond(parseDouble(instantOpsPerSec, 0.0));
            } else {
                commandInfo.setCommandsPerSecond(0.0);
            }
            monitorVO.setCommands(commandInfo);

            // 客户端信息
            RedisMonitorVO.ClientInfo clientInfo = new RedisMonitorVO.ClientInfo();
            clientInfo.setConnectedClients(parseInt(infoMap.get("connected_clients"), 0));
            clientInfo.setBlockedClients(parseInt(infoMap.get("blocked_clients"), 0));
            monitorVO.setClients(clientInfo);

            return monitorVO;
        } catch (Exception e) {
            log.error("获取 Redis 监控信息失败", e);
            RedisMonitorVO vo = new RedisMonitorVO();
            vo.setAvailable(false);
            vo.setMessage("获取 Redis 监控信息失败: " + e.getMessage());
            return vo;
        }
    }
