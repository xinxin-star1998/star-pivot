...
相关配置：`application.yml` / 各环境 profile 中的 `oss.*`（endpoint、access-key、bucket-name 等）。
...
| 实现 | `OssFileStorageService`（唯一实现，注入 `OssUtil`） |
| 底层工具 | `OssUtil.java`（路径、大小、格式、先传后删） |
...
**实现**：`OssFileStorageService` → `OssUtil`（见 `application.yml` 中 `oss.*` 配置）。
...
- 私有桶 OSS 永久 URL 在组件内换 presigned 后展示，尺寸约 40px。
...
1. 数据库存永久 URL 或对象路径。
2. `ArtAvatarDisplay` / `art-avatar-upload` 识别 OSS 域名或对象路径。
...
| `storage/impl/OssFileStorageService.java` | OSS 实现 |
| `utils/OssUtil.java` | 上传/删除/presigned |
...
- [ ] `oss.*` 与当前环境一致（endpoint、bucket、密钥等）
- [ ] 上传文件 **≤ 2MB**，格式为图片
