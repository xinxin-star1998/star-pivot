...
flowchart TB
    ...
    subgraph Biz["业务模块层 Business Module Layer"]
        ...
        M3["star-pivot-quartz<br/>定时任务"]
        M4["扩展模块<br/>dict / monitor 等"]
    end
    ...
    subgraph Framework["框架层 Framework Layer"]
        ...
        FW7["framework-boot-starter 启动聚合"]
    end
...
- **业务模块层**：`star-pivot-system`（用户/角色/菜单/部门等）、`star-pivot-generator`（代码生成）、`star-pivot-quartz`（定时任务），以及 `dict`、`monitor` 等扩展模块。
- **接口/控制层**：Controller 暴露 REST API，调用 Service 与 Mapper，对应实体与 DTO。
- **框架层**：核心、日志、文件、Web、common 聚合、security、boot-starter 等，为业务提供通用能力；**quartz 属于业务模块** `star-pivot-quartz`，由启动模块引入。
